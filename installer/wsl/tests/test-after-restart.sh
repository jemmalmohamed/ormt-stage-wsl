...
if test "$SCOPE" = "auto"; then
  ...
  if has_state stage-validated; then
    stage_detected=true
  elif test -n "$(docker ps -aq --filter label=com.docker.compose.project=ormt-web-stage)"; then
    stage_detected=true
  else
    for container in ormt-core-api ormt-content-api ormt-pdf-renderer; do
